...
var player_in_bath := false

var is_cleaning := false
var press_time := 0.0
var press_threshold := 0.5
...
func _input(event: InputEvent):
	...
	if custom_cursor and event is InputEventMouseMotion:
...
func _process(delta):
	...
	if mouse_held:
		...
		if press_time >= press_threshold and not is_cleaning:
			_start_cleaning()
	...
	if result and result.collider == player:
		...
		_show_feedback(result.position)
...
func _apply_cleaning(delta):

	if not data_manager or not data_manager.is_glucolife_active:
		return

	var increase = hygiene_increase_per_second * delta
